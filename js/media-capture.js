...
const MediaCapture = (() => {
  ...
  function finishVideo() {
    ...
    // Use FileReader base64 — blob: URLs fail on file:// protocol
    const reader = new FileReader();
    reader.onload = function(evt) {
      pendingMedia.push({type:'video', url:evt.target.result, label:'Video '+(pendingMedia.length+1)});
      renderPendingList();
    };
  ...
  function finishAudio() {
    ...
    const reader = new FileReader();
    reader.onload = function(evt) {
      pendingMedia.push({type:'audio', url:evt.target.result, label:'Audio '+(pendingMedia.length+1)});
      renderPendingList();
    };
    reader.readAsDataURL(blob);
